...
\begin{figure}[!tb]
  ...
  box/.style={draw,rectangle,minimum size=2.0cm,text width=5.75cm,align=center}]
    \matrix (confusion_matrix) {
      ...
      [box] {\underline{\gls{tn}} \\ \small{remaining categories correctly classified not as \texttt{A}}};
      \\
    };
...
% Reset the svm glossary as we first talk about it here
\glsreset{svm}
...
\begin{table}[!tb]
  ...
  \begin{threeparttable}
    ...
    \end{tablenotes}
  \end{threeparttable}
  \caption{Basic comparison of different \glsentryname{svm} implementations from a user perspective.}
  \vspace{2mm}
  \hrule
  \label{tab:svm_tools}
\end{table}
