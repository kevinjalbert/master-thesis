...
\begin{table}[h]
  ...
  \vspace{1mm}
  \footnotesize{The mutation operators in \emph{Table~\ref{tab:method_operators} come from the mutation testing tool MuJava~\cite{MO05a}.}}
  \vspace{1mm}
...
\begin{itemize}
  ...
  \item \textbf{Method-Level}: whether a tool has a set of \emph{traditional} method-level mutation operators as mentioned in Section~\ref{subsec:background_method_operators}.
  ...
  \item \textbf{Class-Level}: whether a tool has a set of object-oriented class-level mutation operators as mentioned in Section~\ref{subsec:background_class_operators}.
  ...
  \item \textbf{Structured Output}: whether a tool has support to output results in a structure format (i.e., \gls{xml}, etc \ldots)
...
Many areas of research have benefited from machine learning such as the data mining~\cite{WFH11}, computer vision~\cite{Her03}, biology~\cite{OLP08}, business~\cite{Her00}, health sciences~\cite{Kon01} amongst many others areas. Furthermore, most companies that offer a complex recommendation system utilize machine learning techniques. For example in 2006, \emph{Netflix} challenged the computer science, data mining and machine learning communities to develop a system that exceed its own recommendation system~\cite{BL07}. Several tools that provide sophisticated machine learning techniques in a \emph{off-the-shelf} format does exist such as WEKA~\cite{HFH+09} and LIBSVM~\cite{CL11}. These tools allow other research areas not familiar in machine learning to exploit their benefits.
...
\begin{table}[h]
  ...
  \begin{tabular}{|l|l|}
    ...
    \hline NOT & Number of tests cases \\
    \hline
  \end{tabular}
