...
\begin{table}[!t]
  ...
  \vspace{1mm}
  \footnotesize{\emph{The above metrics are grouped based on similar metrics (i.e., same entity/object) and by the means they are acquired (i.e., the tool used), which resulted in the following feature sets: \ding{172} -- Source Code, \ding{173} -- Coverage, \ding{174} -- Accumulated Source Code, \ding{175} -- Accumulated Test Case.}}
  \vspace{2mm}
  \hrule
  \label{tab:metrics}
\end{table}
...
\begin{sidewaystable}[h]
  \centering
	\captionbox{Extracted coverage test suite metrics (feature set \ding{173} of Table~\ref{tab:metrics}) of the \texttt{Triangle} system.\label{tab:triangle_coverage_metrics}}{
  	\rowcolors{2}{gray!30}{gray!20}
	...
	\centering
	\captionbox{Merged test suite metrics (feature set \ding{175} in Table~\ref{tab:metrics}) for each source code unit of the \texttt{Triangle} system.\label{tab:triangle_merge_test_metrics}}{
  	\rowcolors{2}{gray!30}{gray!20}
	...
	\centering
	\captionbox{Aggregation of method-level source code metrics for each class-level source code unit (feature set \ding{174} of Table~\ref{tab:metrics}) of the \texttt{Triangle} system.\label{tab:triangle_aggregate_metrics}}{
  	\rowcolors{2}{gray!30}{gray!20}
  	\begin{tabular}{|l|r|r|r|r|r|r|r|r|}
    	\hline
    	\rowcolor[RGB]{169,196,223}
