...
\begin{table}[h]
  ...
  \begin{threeparttable}
    \begin{tabular}{|l|l|l|c|}
      ...
      % Set 5: Mutation Metrics
      \hline NCM & Number of covered mutants in code unit & Class/Method & \ding{176} \\
      \hline NCM\emph{x}\tnote{a} & Number of \emph{x} covered mutants in code unit & Class/Method & \ding{176} \\
      \hline
    \end{tabular}
    \begin{tablenotes}
      \item[a] This is a generalization of 7 metrics, the \emph{x} represents the different mutation operators from Table~\ref{tab:javalanche_operators}.
    \end{tablenotes}
  \end{threeparttable}

  \caption{The complete set of metrics used as attributes for each vector of the \gls{svm}.}
  \vspace{1mm}
  \footnotesize{\emph{Table~\ref{tab:metrics} shows the complete set of metrics used as attributes for each vector of the \gls{svm}, categorized by feature set (\ding{172}: Source Code Metrics, \ding{173}: Coverage Metrics, \ding{174}: Accumulated Source Code Metrics, \ding{175}: Accumulated Test Case Metrics, \ding{176}: Mutation Metrics).}}
  \vspace{2mm}
  \hrule
  \label{tab:metrics}
\end{table}
...
Javalanche generates all possible mutants, then considers the set of them that are covered by the provided test suite. Furthermore we utilize the number of covered mutants as another metric set for our attributes (see feature set \ding{176} in Table~\ref{tab:metrics}). We believe that the number of covered mutants will provide useful insight for describing mutation scores of source code units, it also turns out that the generation of mutants is not the costly part of mutation testing. Given the set of covered mutants Javalanche then tests and records the results of each mutant using its subset of covered test cases for that specific mutant. The newly added analyzer for Javalanche then outputs an intermediate \gls{csv} file of mutation scores for the covered source code units. Using the example \texttt{Triangle} system presented in Section~\ref{subsec:approach_inputs} the \gls{csv} file of the acquire mutation scores are shown in Figure~\ref{fig:triangle_mutation_scores}. Using the \gls{csv} file we populate a database with all the acquired data, this eases management and analysis of the data.
